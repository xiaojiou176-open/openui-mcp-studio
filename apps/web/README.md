...
- Frontend runtime is pinned to `Next.js 15.5.14` with `react 19.2.4` and `react-dom 19.2.4` so the app and Radix-driven component tree resolve a single React runtime during build/start smoke validation

## Runtime

- Local entrypoints come from the repository root: `npm run prepare:next-app` and `npm run smoke:e2e`
