...
| `npm run security:github:public:audit` | read-only GitHub public-surface review across open secret-scanning alerts, open code-scanning alerts, heuristic code search, comment surfaces, and fetchable pull refs | does not remove read-only PR refs by itself; GitHub code-search rate limits are recorded as notes rather than repo findings when the primary alert surfaces are already clean |
