...
function resolveLegacyResidueTargets(workspaceRoot) {
	return [path.resolve(workspaceRoot, ".runtime-cache/temp")];
}
...
async function cleanLegacyResidueDirectories(workspaceRoot, dryRun) {
	const legacyTargets = resolveLegacyResidueTargets(workspaceRoot);

	if (dryRun) {
		for (const targetPath of legacyTargets) {
			globalThis.console.log(
				`[clean:runtime] would remove legacy residue dir: ${targetPath}`,
			);
		}
		return legacyTargets.length;
	}
...
async function cleanRuntime(argv = runtimeProcess.argv.slice(2)) {
	...
	const legacyResidueTargets = resolveLegacyResidueTargets(workspaceRoot);
	if (shouldIncludeE2EArtifacts(argv)) {
		targetDirs.push(...resolveExtendedTargetDirs(workspaceRoot));
	}

	await Promise.all(
		[...targetDirs.map((targetDir) => targetDir.path), ...legacyResidueTargets].map(
			(targetPath) =>
	...
	if (isDryRunEnabled(argv)) {
		...
		await cleanLegacyResidueDirectories(workspaceRoot, true);
		return;
	}
	...
	const legacyResidueRemoved = await cleanLegacyResidueDirectories(
		workspaceRoot,
		false,
	);
	if (
		stressLeakRemoved > 0 ||
		browserTempRemoved > 0 ||
		logFilesRemoved > 0 ||
		legacyResidueRemoved > 0
	) {
		globalThis.console.log(
			`[clean:runtime] additional cleanup: stress-leak dirs=${stressLeakRemoved}, browser tmp dirs=${browserTempRemoved}, log files=${logFilesRemoved}, legacy residue dirs=${legacyResidueRemoved}`,
		);
	}
}
