...
async function initializeRunLayout(layout, commandArgs) {
	const workspaceRoot = path.resolve(layout.rootDir ?? process.cwd());
	...
	for (const relativePath of requiredDirectories) {
		await mkdir(path.resolve(workspaceRoot, relativePath), { recursive: true });
	}
	for (const relativeLogPath of Object.values(layout.logPathsByChannel)) {
		const absoluteLogPath = path.resolve(workspaceRoot, relativeLogPath);
...
async function ensureRunManifest(layout, commandArgs = []) {
	...
	const runManifestPath = path.resolve(workspaceRoot, layout.runManifestPathRelative);
...
async function writeCiLifecycleLog(layout, payload) {
	const workspaceRoot = path.resolve(layout.rootDir ?? process.cwd());
	const targetPath = path.resolve(workspaceRoot, layout.logPathsByChannel.ci);
