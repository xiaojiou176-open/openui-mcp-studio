...
function assertOutputPathIsSafe(outputPath, workspaceRoot) {
	...
	const resolvedPath = path.resolve(resolvedWorkspaceRoot, outputPath);
...
async function resolveSafeOutputWriteTarget(outputPath, workspaceRoot) {
	assertOutputPathIsSafe(outputPath, workspaceRoot);

	const resolvedWorkspaceRoot = await realpath(resolveWorkspaceRoot(workspaceRoot));
	const allowedRoot = path.resolve(resolvedWorkspaceRoot, SUMMARY_ROOT_DIR);
	const resolvedPath = path.resolve(resolvedWorkspaceRoot, outputPath);
...
async function writeQualityScoreFile(outputPath, report, options = {}) {
	const resolvedPath = await resolveSafeOutputWriteTarget(
		outputPath,
