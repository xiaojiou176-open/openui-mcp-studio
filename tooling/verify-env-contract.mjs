...
const ENV_KEY_PATTERN =
	/^(?:(?:OPENUI|GEMINI)_[A-Z0-9_]+|NEXT_PUBLIC_SITE_URL)$/u;

function isValidCanonicalSiteUrl(raw) {
...
async function verifyLocalEnvConsistency(rootDir, contractKeys) {
	...
	try {
		...
		const siteUrl = (envMap.get("NEXT_PUBLIC_SITE_URL") ?? "").trim();
		if (!isValidCanonicalSiteUrl(siteUrl)) {
			issues.push(
				"- .env NEXT_PUBLIC_SITE_URL must be empty or an absolute http/https URL.",
			);
		}
