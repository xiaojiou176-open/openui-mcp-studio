...
const DOCTOR_CHECKS = [
	...
	{ id: "deliverySurface", command: ["npm", "run", "-s", "governance:delivery-surface:check"] },
	...
	{ id: "hostSafety", command: ["npm", "run", "-s", "host:safety:check"] },
...
async function runRepoDoctorCli({
	...
	browserLaneProvider = async (cwd, env) => {
		const { contract } = await readSpaceGovernanceContract({ rootDir: cwd });
		return describeBrowserLanePolicy(cwd, contract, { env });
	},
	cwd = process.cwd(),
	env = process.env,
	...
	const browserLane = await browserLaneProvider(cwd, env);
