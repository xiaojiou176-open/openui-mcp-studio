...
- Keep process execution worker-safe:
  - never send signals to negative pids or process groups
  - never widen teardown from a child handle to host-scoped primitives
  - only use recorded positive pids for liveness probes
