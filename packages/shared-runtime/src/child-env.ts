...
export const SHADCN_BRIEF_CHILD_ENV_BASE_ALLOWLIST = Object.freeze([
	"PATH",
	"COMSPEC",
	"SYSTEMROOT",
	"HOME",
...
function isExplicitlyAllowedByBaseAllowlist(
	...
	const normalizedKey = normalizeEnvKeyForMatch(key, caseInsensitiveKeys);
	return SHADCN_BRIEF_CHILD_ENV_BASE_ALLOWLIST.some(
		(token) =>
			normalizeEnvKeyForMatch(token, caseInsensitiveKeys) === normalizedKey,
	);
}
...
function parseCustomAllowlist(raw: string | undefined): string[] {
	...
	for (const chunk of raw.split(",")) {
		...
		if (
			...
			throw new Error(
				`SHADCN_BRIEF_CHILD_ENV_ALLOWLIST contains invalid token: "${token}". ${CHILD_ENV_ALLOWLIST_FORMAT_HINT}`,
			);
		}
...
export function buildChildEnvFromAllowlist(
	...
	allowlistRaw: string | undefined = sourceEnv.SHADCN_BRIEF_CHILD_ENV_ALLOWLIST,
	options?: {
		caseInsensitiveKeys?: boolean;
	},
): NodeJS.ProcessEnv {
