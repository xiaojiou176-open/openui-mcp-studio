...
export class IdempotencyStore {
	...
	private async acquireLock(lockPath: string): Promise<() => Promise<void>> {
		const start = this.now();
		let attemptedImmediateStaleProbe = false;
		while (true) {
			try {
				...
				if (!attemptedImmediateStaleProbe) {
					attemptedImmediateStaleProbe = true;
					const clearedImmediately = await this.tryClearStaleLock(lockPath);
		...
		try {
			...
			if (typeof ownedInode === "number") {
				...
				if (currentStat.ino !== ownedInode) {
					// Container/overlay filesystems can churn inode numbers even when
					// the same lock owner still holds the file. Owner match stays the
					// stronger safety check, so do not treat inode drift alone as a blocker.
				}
			}
	...
	private startLeaseHeartbeat(
		...
		const interval = setInterval(() => {
			...
			if (inFlightRenewals.size > 0) {
				return;
			}
			const renewal = this.renewExecutionLease(
				idempotencyKey,
				ownerId,
				leaseMs,
			)
