...
## 5.1 Shared-Machine Browser And Container Hygiene

Claude must treat browser state and Docker residue as limited shared-machine resources:

- do not open extra Chrome/Chromium/Playwright browser instances when an existing repo-owned lane is enough
- do not reuse tabs, windows, profiles, or logged-in sessions opened by another repo or another L1
- close repo-owned tabs, windows, browser sessions, and temporary profile clones when the task finishes
- avoid growing task-only profile/cache directories without a cleanup plan
- clean disposable containers, temporary builders, and task-only Docker residue when the task no longer needs them
- do not sweep shared machine-level Docker/browser caches unless the task explicitly covers machine-level maintenance

## 5.2 External Account Write Boundary

- Default to read-only on external dashboards, marketplaces, consoles, browser-authenticated products, and other accounts.
- Do not perform external write actions unless the user explicitly asked for that write and it is required for the current task.
- Treat publish, submit, approve, billing, and account-setting changes as high-risk external writes even when the browser is already logged in.
