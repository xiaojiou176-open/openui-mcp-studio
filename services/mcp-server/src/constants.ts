...
export const DEFAULT_SHADCN_BRIEF_LOG_LEVEL = String(
	resolveEnvDefaultValue("SHADCN_BRIEF_LOG_LEVEL"),
);
export const DEFAULT_SHADCN_BRIEF_LOG_OUTPUT = String(
	resolveEnvDefaultValue("SHADCN_BRIEF_LOG_OUTPUT"),
);
export const DEFAULT_SHADCN_BRIEF_LOG_ROTATE_ON_START = String(
	resolveEnvDefaultValue("SHADCN_BRIEF_LOG_ROTATE_ON_START"),
);
export const DEFAULT_SHADCN_BRIEF_LOG_DIR = String(
	resolveEnvDefaultValue("SHADCN_BRIEF_LOG_DIR"),
);
export const DEFAULT_SHADCN_BRIEF_CACHE_DIR = String(
	resolveEnvDefaultValue("SHADCN_BRIEF_CACHE_DIR"),
);
export const DEFAULT_SHADCN_BRIEF_LOG_RETENTION_DAYS = Number(
	resolveEnvDefaultValue("SHADCN_BRIEF_LOG_RETENTION_DAYS"),
);
export const DEFAULT_SHADCN_BRIEF_LOG_MAX_FILE_MB = Number(
	resolveEnvDefaultValue("SHADCN_BRIEF_LOG_MAX_FILE_MB"),
);
...
export const DEFAULT_SHADCN_BRIEF_CACHE_RETENTION_DAYS = Number(
	resolveEnvDefaultValue("SHADCN_BRIEF_CACHE_RETENTION_DAYS"),
);
export const DEFAULT_SHADCN_BRIEF_CACHE_MAX_BYTES = Number(
	resolveEnvDefaultValue("SHADCN_BRIEF_CACHE_MAX_BYTES"),
);
export const DEFAULT_SHADCN_BRIEF_CACHE_CLEAN_INTERVAL_MINUTES = Number(
	resolveEnvDefaultValue("SHADCN_BRIEF_CACHE_CLEAN_INTERVAL_MINUTES"),
);
export const DEFAULT_OPENUI_TOOL_CACHE_ROOT = String(
	resolveEnvDefaultValue("OPENUI_TOOL_CACHE_ROOT"),
);
export const DEFAULT_OPENUI_TOOL_CACHE_RETENTION_DAYS = Number(
	resolveEnvDefaultValue("OPENUI_TOOL_CACHE_RETENTION_DAYS"),
);
export const DEFAULT_OPENUI_TOOL_CACHE_MAX_BYTES = Number(
	resolveEnvDefaultValue("OPENUI_TOOL_CACHE_MAX_BYTES"),
);
export const DEFAULT_OPENUI_TOOL_CACHE_CLEAN_INTERVAL_MINUTES = Number(
	resolveEnvDefaultValue("OPENUI_TOOL_CACHE_CLEAN_INTERVAL_MINUTES"),
);
export const DEFAULT_OPENUI_CHROME_USER_DATA_DIR = String(
	resolveEnvDefaultValue("OPENUI_CHROME_USER_DATA_DIR"),
);
export const DEFAULT_OPENUI_CHROME_PROFILE_DIRECTORY = String(
	resolveEnvDefaultValue("OPENUI_CHROME_PROFILE_DIRECTORY"),
);
export const DEFAULT_OPENUI_CHROME_CHANNEL = String(
	resolveEnvDefaultValue("OPENUI_CHROME_CHANNEL"),
);
export const DEFAULT_OPENUI_CHROME_EXECUTABLE_PATH = String(
	resolveEnvDefaultValue("OPENUI_CHROME_EXECUTABLE_PATH"),
);
export const DEFAULT_OPENUI_CHROME_CDP_PORT = Number(
	resolveEnvDefaultValue("OPENUI_CHROME_CDP_PORT"),
);
export const DEFAULT_OPENUI_HOSTED_API_HOST = String(
	resolveEnvDefaultValue("OPENUI_HOSTED_API_HOST"),
);
export const DEFAULT_OPENUI_HOSTED_API_PORT = Number(
	resolveEnvDefaultValue("OPENUI_HOSTED_API_PORT"),
);
export const DEFAULT_OPENUI_HOSTED_API_MAX_REQUESTS_PER_MINUTE = Number(
	resolveEnvDefaultValue("OPENUI_HOSTED_API_MAX_REQUESTS_PER_MINUTE"),
);
...
function expandHomePath(filePath: string, homeDir = os.homedir()): string {
...
export function getOpenuiMcpLogLevel(): OpenuiMcpLogLevel {
	const raw = process.env.SHADCN_BRIEF_LOG_LEVEL;
	if (raw === undefined || raw.trim() === "") {
		return DEFAULT_SHADCN_BRIEF_LOG_LEVEL as OpenuiMcpLogLevel;
	}
	...
	throw new Error(
		`SHADCN_BRIEF_LOG_LEVEL must be one of "debug" | "info" | "warn" | "error", received: ${JSON.stringify(raw)}.`,
	);
}

export function getOpenuiMcpLogOutput(): OpenuiMcpLogOutput {
	const raw = process.env.SHADCN_BRIEF_LOG_OUTPUT;
	if (raw === undefined || raw.trim() === "") {
		return DEFAULT_SHADCN_BRIEF_LOG_OUTPUT as OpenuiMcpLogOutput;
	}
	...
	throw new Error(
		`SHADCN_BRIEF_LOG_OUTPUT must be one of "stderr" | "file" | "both", received: ${JSON.stringify(raw)}.`,
	);
}

export function getOpenuiMcpLogRotateOnStart(): OpenuiMcpLogRotateOnStart {
	const raw = process.env.SHADCN_BRIEF_LOG_ROTATE_ON_START;
	if (raw === undefined || raw.trim() === "") {
		return DEFAULT_SHADCN_BRIEF_LOG_ROTATE_ON_START as OpenuiMcpLogRotateOnStart;
	}
	...
	throw new Error(
		`SHADCN_BRIEF_LOG_ROTATE_ON_START must be "on" or "off", received: ${JSON.stringify(raw)}.`,
	);
}
...
export function getOpenuiMcpCacheDir(): string {
	const raw = process.env.SHADCN_BRIEF_CACHE_DIR;
	const trimmed = raw?.trim();
	const workspaceRoot = getWorkspaceRoot();
	const candidate = trimmed || DEFAULT_SHADCN_BRIEF_CACHE_DIR;
...
function resolveRuntimeDirWithinWorkspace(
	envName: "SHADCN_BRIEF_LOG_DIR" | "SHADCN_BRIEF_CACHE_DIR",
	defaultDir: string,
): string {
	const raw = process.env[envName];
	const trimmed = raw?.trim();
	...
	if (!isPathInsideRootWithRealpath(workspaceRoot, resolved)) {
		throw new Error(
			`${envName} must resolve inside SHADCN_BRIEF_WORKSPACE_ROOT (${workspaceRoot}), received: ${JSON.stringify(
				raw,
			)}.`,
		);
	}
...
export function getOpenuiMcpLogRetentionDays(): number {
	return requirePositiveIntegerFromEnv(
		...
		DEFAULT_SHADCN_BRIEF_LOG_RETENTION_DAYS,
	);
}
...
export function getOpenuiMcpLogMaxFileMb(): number {
	return requirePositiveNumberFromEnv(
		...
		DEFAULT_SHADCN_BRIEF_LOG_MAX_FILE_MB,
	);
}
...
export function getOpenuiMcpCacheCleanIntervalMinutes(): number {
	return requirePositiveIntegerFromEnv(
		...
		DEFAULT_SHADCN_BRIEF_CACHE_CLEAN_INTERVAL_MINUTES,
	);
}

export function getOpenuiToolCacheRoot(): string {
	const raw = process.env.OPENUI_TOOL_CACHE_ROOT?.trim();
	const candidate = raw || DEFAULT_OPENUI_TOOL_CACHE_ROOT;
	const expanded = expandHomePath(candidate);
	return path.isAbsolute(expanded)
		? path.resolve(expanded)
		: path.resolve(getWorkspaceRoot(), expanded);
}

export function getOpenuiToolCacheRetentionDays(): number {
	return requirePositiveIntegerFromEnv(
		"OPENUI_TOOL_CACHE_RETENTION_DAYS",
		DEFAULT_OPENUI_TOOL_CACHE_RETENTION_DAYS,
	);
}

export function getOpenuiToolCacheMaxBytes(): number {
	return requirePositiveIntegerFromEnv(
		"OPENUI_TOOL_CACHE_MAX_BYTES",
		DEFAULT_OPENUI_TOOL_CACHE_MAX_BYTES,
	);
}

export function getOpenuiToolCacheCleanIntervalMinutes(): number {
	return requirePositiveIntegerFromEnv(
		"OPENUI_TOOL_CACHE_CLEAN_INTERVAL_MINUTES",
		DEFAULT_OPENUI_TOOL_CACHE_CLEAN_INTERVAL_MINUTES,
	);
}

export function getOpenuiChromeUserDataDir(): string | null {
	const raw = process.env.OPENUI_CHROME_USER_DATA_DIR?.trim();
	if (!raw) {
		return null;
	}
...
export function getOpenuiChromeProfileDirectory(): string | null {
	const value = process.env.OPENUI_CHROME_PROFILE_DIRECTORY?.trim();
	return value ? value : null;
}

export function getOpenuiChromeChannel(): string {
	const value = process.env.OPENUI_CHROME_CHANNEL?.trim();
	return value || DEFAULT_OPENUI_CHROME_CHANNEL;
}

export function getOpenuiChromeExecutablePath(): string | null {
	const raw = process.env.OPENUI_CHROME_EXECUTABLE_PATH?.trim();
	if (!raw) {
		return null;
	}
	const expanded = expandHomePath(raw);
...
export function getOpenuiChromeCdpPort(): number {
	return requirePositiveIntegerFromEnv(
...
export function getOpenuiHostedApiHost(): string {
	const raw = process.env.OPENUI_HOSTED_API_HOST;
	if (raw === undefined || raw.trim() === "") {
		return DEFAULT_OPENUI_HOSTED_API_HOST;
	}
	return raw.trim();
}

export function getOpenuiHostedApiPort(): number {
	return requirePositiveIntegerFromEnv(
...
export function getOpenuiHostedApiBearerToken(): string {
	const raw = process.env.OPENUI_HOSTED_API_BEARER_TOKEN;
	...
	throw new Error(
		"OPENUI_HOSTED_API_BEARER_TOKEN must be configured and non-empty when the hosted API runtime is started.",
	);
}

export function getOpenuiHostedApiMaxRequestsPerMinute(): number {
	return requirePositiveIntegerFromEnv(
...
export function validateOpenuiRuntimeConfig(): void {
	...
	parseChildEnvAllowlist(process.env.SHADCN_BRIEF_CHILD_ENV_ALLOWLIST);
	getWorkspaceRoot();
	...
	getOpenuiToolCacheRoot();
	getOpenuiToolCacheRetentionDays();
	getOpenuiToolCacheMaxBytes();
	getOpenuiToolCacheCleanIntervalMinutes();
	getOpenuiChromeCdpPort();
}

export function validateOpenuiHostedApiConfig(): void {
	getOpenuiHostedApiHost();
	getOpenuiHostedApiPort();
	getOpenuiHostedApiBearerToken();
	getOpenuiHostedApiMaxRequestsPerMinute();
}
