...
const UiuxReviewInputSchema = z.object({
	...
	auditScope: z.enum(["snippet", "page", "workspace"]).default("page"),
	auditTarget: z.string().min(1).default("provided-html"),
...
async function requestModelReview(input: {
	...
	const prompt = [
		...
		"- category must be one of hierarchy, consistency, design_system, interaction_clarity, accessibility.",
...
export function registerUiuxReviewTool(server: McpServer): void {
	server.registerTool(
		...
		async (args) => {
			...
			const audit = buildUiuxAuditFrame({
