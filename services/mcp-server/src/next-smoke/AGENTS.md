...
- Keep shutdown worker-safe:
  - do not use negative pid or process-group signals
  - only terminate the exact spawned child process tracked by this module
  - if the pid is missing or invalid, fail closed instead of widening the kill scope
