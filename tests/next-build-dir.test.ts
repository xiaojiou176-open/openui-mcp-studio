...
describe("next build dir resolution", () => {
	...
	it("reads distDir from a TypeScript config file when earlier candidates are missing", async () => {
		const root = await createRoot();
		await fs.writeFile(
			path.resolve(root, "next.config.ts"),
			"export default { distDir: 'ts-output' };\n",
			"utf8",
		);

		await expect(resolveNextBuildDir(root)).resolves.toBe(
			path.resolve(root, "ts-output"),
		);
	});
	...
	it("falls back to .next when distDir is an absolute path outside the project root", () => {
		...
		).toBe(path.resolve(root, ".next"));
	});

	it("falls back to .next when config files exist but none declare distDir", async () => {
		const root = await createRoot();
		await installCustomNextModules({
			root,
			constantsSource: "module.exports = { OTHER_PHASE: 'dev-phase' };",
