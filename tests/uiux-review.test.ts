...
describe("uiux review tool", () => {
	...
	it("returns audit frame with style-pack summary and category rollups", async () => {
		const harness = createToolHarness();
		registerUiuxReviewTool(harness.server);
		...
		const payload = JSON.parse(readText(result)) as {
			audit: {
				...
				stylePack: {
					id: string;
					label: string;
					contract: { tokenMode: string };
				};
				automatedSignals: { verdict: string; sourceKinds: string[] };
				manualReview: { required: boolean; focusAreas: string[] };
				nextOperatorMove: { priority: string; category: string } | null;
				categories: Array<{ id: string; status: string }>;
				nextSteps: Array<{ priority: string; category: string }>;
			};
		};

		expect(payload.audit.scope).toBe("page");
		...
		expect(payload.audit.automatedSignals.verdict).toBe("fail");
		expect(payload.audit.automatedSignals.sourceKinds).toEqual(["heuristic"]);
		expect(payload.audit.manualReview.required).toBe(true);
		expect(payload.audit.manualReview.focusAreas.length).toBeGreaterThan(0);
		expect(payload.audit.nextOperatorMove).toEqual(
			expect.objectContaining({
				priority: "now",
			}),
		);
		expect(payload.audit.categories).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ id: "hierarchy" }),
				expect.objectContaining({ id: "design_system" }),
				expect.objectContaining({ id: "accessibility" }),
			]),
		);
		expect(payload.audit.nextSteps.length).toBeGreaterThan(0);
	});

	it("respects alternate style pack selection in audit output", async () => {
		const harness = createToolHarness();
		registerUiuxReviewTool(harness.server);
		...
		const payload = JSON.parse(readText(result)) as {
			audit: {
				...
				stylePack: {
					id: string;
					emphasis: string;
					contract: { primaryActionRule: string };
				};
				manualReview: { required: boolean };
		...
		expect(payload.audit.manualReview.required).toBe(true);
		expect(payload.audit.categories[0]?.id).toBe("interaction_clarity");
	});
