...
const ORIGINAL_CHILD_ENV_ALLOWLIST = process.env.SHADCN_BRIEF_CHILD_ENV_ALLOWLIST;
...
describe("ci gate orchestration", () => {
	afterEach(() => {
		if (ORIGINAL_CHILD_ENV_ALLOWLIST === undefined) {
			...
			process.env.SHADCN_BRIEF_CHILD_ENV_ALLOWLIST = ORIGINAL_CHILD_ENV_ALLOWLIST;
		}
	...
	it("passes allowlisted child env instead of full host env", async () => {
		...
		process.env.SHADCN_BRIEF_CHILD_ENV_ALLOWLIST = "TEST_ONLY_VALUE";
	...
	it("enforces fast-gates before deep-gates and keeps browser matrix parallel", () => {
		...
		expect(stage1?.tasks.some((task) => task.id === "hostSafety")).toBe(true);
		expect(
			stage1?.tasks.find((task) => task.id === "hostSafety")?.command,
		).toBe("node tooling/check-host-safety.mjs");
	...
	it("rejects summary-file directory symlink that escapes ci-gate runtime directory", async () => {
		await withTempWorkspace(async (workspaceRoot) => {
			...
			try {
				...
				await expect(
					writeSummaryFile(
						".runtime-cache/runs/test-run/summary.json",
						{ ok: true },
						{ workspaceRoot },
					),
