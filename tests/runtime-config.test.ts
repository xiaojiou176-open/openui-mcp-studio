...
function setValidRuntimeEnv() {
	...
	process.env.OPENUI_CHROME_PROFILE_DIRECTORY = "";
...
describe("runtime config guardrails", () => {
	...
	it("resolves tool cache root and numeric policy values from env", async () => {
		setValidRuntimeEnv();
		...
		expect(getOpenuiToolCacheRoot()).toContain(
			"/.cache/openui-mcp-studio/tooling",
		);
		expect(getOpenuiToolCacheRetentionDays()).toBe(3);
		expect(getOpenuiToolCacheMaxBytes()).toBe(5_368_709_120);
		expect(getOpenuiToolCacheCleanIntervalMinutes()).toBe(60);
	});

	it("resolves chrome profile getters for missing, absolute, and relative values", async () => {
		setValidRuntimeEnv();
		const {
			getOpenuiChromeUserDataDir,
			getOpenuiChromeProfileDirectory,
			getOpenuiChromeChannel,
			getOpenuiChromeExecutablePath,
			getOpenuiChromeCdpPort,
		} = await loadConstantsModule();

		expect(getOpenuiChromeUserDataDir()).toBeNull();
		expect(getOpenuiChromeProfileDirectory()).toBeNull();
		expect(getOpenuiChromeChannel()).toBe("chrome");
		expect(getOpenuiChromeExecutablePath()).toBeNull();
		...
		expect(getOpenuiChromeExecutablePath()).toBe(
			`${process.cwd().replaceAll("\\", "/")}/bin/chrome`,
		);
		expect(getOpenuiChromeCdpPort()).toBe(9555);
	});
});
