...
function createTempCacheDir(): string {
	const cacheDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "shadcn-brief-cache-retention-"),
	);
...
describe("cache retention", () => {
	...
	it("resolves retention config from env and rejects invalid numeric values", () => {
		process.env.SHADCN_BRIEF_CACHE_DIR = " ./tmp-cache ";
		process.env.SHADCN_BRIEF_CACHE_RETENTION_DAYS = "7";
		process.env.SHADCN_BRIEF_CACHE_MAX_BYTES = "2048";
		process.env.SHADCN_BRIEF_CACHE_CLEAN_INTERVAL_MINUTES = "30";
		...
		process.env.SHADCN_BRIEF_CACHE_MAX_BYTES = "0";
		expect(() => resolveCacheRetentionConfigFromEnv()).toThrow(
			/SHADCN_BRIEF_CACHE_MAX_BYTES must be a positive integer/,
		);
	});
	...
	it("rejects cache dir that resolves outside workspace root", () => {
		...
		process.env.SHADCN_BRIEF_CACHE_DIR = outsideTarget;
		process.env.SHADCN_BRIEF_CACHE_RETENTION_DAYS = "7";
		process.env.SHADCN_BRIEF_CACHE_MAX_BYTES = "2048";
		process.env.SHADCN_BRIEF_CACHE_CLEAN_INTERVAL_MINUTES = "30";
	...
	it("supports dry-run mode and records ttl/capacity path receipts without deleting files", () => {
		const cacheDir = createTempCacheDir();
		const expiredFilePath = path.join(cacheDir, "expired", "old.cache");
		const oldFilePath = path.join(cacheDir, "old", "large.cache");
		const freshFilePath = path.join(cacheDir, "fresh", "kept.cache");
		const now = Date.now();
		...
		expect(summary.removedExpiredFiles).toBe(1);
		expect(summary.removedCapacityFiles).toBe(1);
		expect(summary.removedExpiredPaths).toEqual([expiredFilePath]);
		expect(summary.removedCapacityPaths).toEqual([oldFilePath]);
		expect(summary.bytesBefore).toBe(190);
		expect(summary.bytesAfter).toBe(40);
		expect(fs.existsSync(expiredFilePath)).toBe(true);
