...
describe("space maintain", () => {
	it("plans and applies repo-local maintenance without touching manual opt-in install surfaces", async () => {
		...
		try {
			...
			expect(dryRun.excludedExternalTargets).toEqual(
			...
			expect(dryRun.excludedPersistentBrowserAssets).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						id: "chrome-user-data-root",
						applyMode: "report-only",
			...
			expect(apply.excludedExternalTargets).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						id: "playwright",
						applyMode: "managed",
			...
			expect(apply.excludedPersistentBrowserAssets).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						id: "chrome-user-data-root",
						applyMode: "report-only",
