...
const ORIGINAL_WORKSPACE_ROOT = process.env.SHADCN_BRIEF_WORKSPACE_ROOT;
...
afterEach(async () => {
	if (ORIGINAL_WORKSPACE_ROOT === undefined) {
		...
		process.env.SHADCN_BRIEF_WORKSPACE_ROOT = ORIGINAL_WORKSPACE_ROOT;
	}
	await Promise.all(
		tempDirs
			.splice(0)
...
describe("runNextSmoke", () => {
	...
	it("uses external target root when valid and reports full successful flow", async () => {
		...
		process.env.SHADCN_BRIEF_WORKSPACE_ROOT = root;
