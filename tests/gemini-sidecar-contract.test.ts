...
const SIDECAR_CONTRACT_TIMEOUT_MS = 60_000;
const SIDECAR_HEALTH_REQUEST_TIMEOUT_MS = 30_000;
...
describe("gemini sidecar contract", () => {
	it(
		"returns JSON-RPC health response",
		async () => {
		...
		SIDECAR_CONTRACT_TIMEOUT_MS,
	);

	it(
		"handles concurrent start/stop calls without lifecycle race failures",
		async () => {
			const scriptPath = path.resolve(
				process.cwd(),
				"services/gemini-sidecar/server.py",
