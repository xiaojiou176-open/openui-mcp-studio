...
describe("clean runtime script", () => {
	it("cleans runtime folders and preserves directory structure", async () => {
		...
		try {
			...
			await Promise.all([
				...
				createFile(legacyResidueFile, "legacy"),
			]);
			...
			await expect(
				fs.stat(path.join(tempRoot, ".runtime-cache", "temp")),
			).rejects.toMatchObject({ code: "ENOENT" });
	...
	it("supports --dry-run and only reports target directories", async () => {
		...
		const legacyResidueDir = path.join(tempRoot, ".runtime-cache", "temp");

		try {
			await writeRuntimePathRegistry(tempRoot);
			await Promise.all([
				...runtimeTargets.map((filePath) => createFile(filePath, "data")),
				createFile(path.join(legacyResidueDir, "legacy.txt"), "legacy"),
			]);
			...
			expect(stdout).toContain(
				"[clean:runtime] would remove legacy residue dir:",
			);
			expect(stdout).toContain(path.join(".runtime-cache", "temp"));
	...
	it("removes legacy .runtime-cache/temp residue without treating it as canonical runtime", async () => {
		...
		try {
			await writeRuntimePathRegistry(tempRoot);
			await createFile(
				path.join(tempRoot, ".runtime-cache", "temp", "legacy", "trace.txt"),
				"legacy",
			);
			...
			expect(stdout).toContain("legacy residue dir");
