...
describe("idempotency store uncovered branches", () => {
	...
	it("still clears lock when owner matches but inode guard drifts", async () => {
		...
		await expect(
			store.compareAndDeleteLock(targetLockPath, "owner-with-inode-drift", 123),
		).resolves.toBe(true);
		await expect(fs.stat(targetLockPath)).rejects.toThrow();
	});
