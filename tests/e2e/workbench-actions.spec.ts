...
async function gotoWorkbench(input: {
	...
	await input.page.goto(resolveWorkbenchUrl(input.baseURL), {
		waitUntil: "load",
	});
...
test("ops panel controls publish visible feedback for promote and signal refresh actions", async ({
	...
	await expect(page.getByTestId("draft-prompt-input")).toHaveValue(
		/Promote next operator packet: Starter bundle truth sync\./i,
	);
	await page.getByRole("button", { name: "Cancel" }).click();

	await page.getByTestId("refresh-signals").click();
	await expect(page.getByTestId("success-state")).toContainText(
		"Proof and review signals refreshed for the pipeline lane.",
	);
...
test("work item action buttons open the contextual draft dialog instead of acting like no-op controls", async ({
	...
	await page.getByTestId("work-item-action-brief-129").click();
	await expect(page.getByTestId("create-draft-dialog")).toBeVisible();
	await expect(page.getByTestId("draft-prompt-input")).toHaveValue(
		/Continue packet: Starter bundle truth sync/i,
	);
	await activateControl(page.getByRole("button", { name: "Cancel" }));

	await page.getByTestId("tab-release").click();
	await page.getByTestId("work-item-action-ship-903").click();
	await expect(page.getByTestId("create-draft-dialog")).toBeVisible();
	await expect(page.getByTestId("draft-prompt-input")).toHaveValue(
		/Run release gates: Public distribution proof shelf/i,
	);
});
