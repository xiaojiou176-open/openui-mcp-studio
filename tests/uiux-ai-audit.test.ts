...
import {
	buildWorkspaceAuditFileHotspots,
	buildWorkspaceAuditFrame,
...
describe("uiux ai audit strict gate", () => {
	...
	it("builds workspace hotspots grouped by file and mapped audit categories", () => {
		const hotspots = buildWorkspaceAuditFileHotspots([
			{
				file: "apps/web/app/page.tsx",
				severity: "error",
				category: "design_system",
			...
			},
			{
				...
				fix: "Demote secondary action",
				confidence: "medium",
				impact: "medium",
				evidenceSnippet: "btn-primary btn-primary",
				source: "model",
			},
			{
				...
				severity: "warning",
				category: "responsive",
				message: "Card wraps awkwardly",
				fix: "Adjust breakpoint rhythm",
				confidence: "medium",
				impact: "medium",
				evidenceSnippet: "grid-cols-4",
				source: "model",
			},
		]);
		...
		expect(hotspots).toEqual([
			{
				file: "apps/web/app/page.tsx",
				issueCount: 2,
				categories: ["design_system", "interaction_clarity"],
			},
			{
				file: "apps/web/components/banner.tsx",
				issueCount: 1,
	...
	it("builds workspace audit frame with style-pack summary, categories, hotspots, and next steps", () => {
		const frame = buildWorkspaceAuditFrame({
			targetRoot: "apps/web",
			auditableFileCount: 3,
			issues: [
				{
					file: "apps/web/app/page.tsx",
					severity: "error",
					category: "design_system",
					message: "Hardcoded colors bypass tokens",
					fix: "Use semantic tokens",
					confidence: "high",
					impact: "high",
					evidenceSnippet: "#777777",
					source: "model",
				},
				{
					file: "apps/web/app/page.tsx",
					severity: "warning",
					category: "usability",
					message: "Primary action overload",
					fix: "Keep one dominant CTA",
					confidence: "high",
					impact: "medium",
					evidenceSnippet: "btn-primary",
					source: "model",
				},
			],
		});
		...
		expect(frame.auditableFileCount).toBe(3);
		...
		expect(frame.manualReview).toEqual({
			...
			focusAreas: ["design_system", "interaction_clarity"],
		});
		...
		expect(frame.nextSteps.length).toBeGreaterThan(0);
	});

	it("respects explicit style-pack selection for workspace audit frames", () => {
		const frame = buildWorkspaceAuditFrame({
			targetRoot: "apps/web",
			...
			issues: [
				{
					...
					severity: "warning",
					category: "usability",
					message: "Current action and next step compete for attention",
					fix: "Keep one dominant operator action",
					confidence: "high",
					impact: "medium",
					evidenceSnippet: "Run audit / Promote draft",
					source: "model",
				},
			],
		});
		...
		expect(frame.automatedSignals.sourceKinds).toEqual(["model"]);
		expect(frame.manualReview.required).toBe(true);
		expect(frame.categories[0]?.id).toBe("interaction_clarity");
	});
